...
class LogicAppsScanner(BaseScanner):
        ...
        for sub in self.get_subscriptions(subscription_ids):
            try:
                ...
                for app in client.web_apps.list():
                    if "workflowapp" not in (app.kind or "").lower():
                        continue
                    for binding in (app.host_name_ssl_states or []):
                        ...
                        certs.append(Certificate(
                            ...
                            resource_type=ResourceType.LOGIC_APPS,
                ...
                logger.error(f"Logic Apps scan failed for subscription {sub.subscription_id}: {e}")
        return certs
